...
def place_random(board):
    ...
    elif generated >= 60 and generated < 97:
        to_place = "4"
...
def swipe_left(board):
    ...
    # by default, the game has not taken any actions
    action_taken = False

    # finds the length of the board
    N = len(board)

    # nested loop, tests if x and y are in the board
    for y in range(N):
        for x in range(N):
            # get the current piece through a loop and the value of the piece one square to the left
            piece_at_xy = get_piece(x, y, board)
            left_adjacent = get_piece(x-1, y, board)

            # if the piece is a star (not 2, 4, 8...) continue the loop
            if piece_at_xy == '*':
                continue

            # if there is no piece to the left, continue the loop (no action)
            if left_adjacent == None:
                continue

            # if the piece is a number, move (returns if the action was executed)
            # need to add "or action_taken" or else it will convert back to false
            action_taken = move(x, y, "left", board) or action_taken


    # at the end of the loop if an action is taken, print the board and add a new piece
    if action_taken:
        end_move(board)
...
def swap(board):
    ...
    if (swap_possible(board) == False):
        ...
        while not (found):
            ...
            if (get_piece(random_x, random_y, board) != '*'):
                found = True
                x1 = random_x
                y1 = random_y
        ...
        while not (found2):
            ...
            random2 = get_piece(random_x, random_y, board)

            if (random2 != '*') and (random2 != get_piece(x1, y1, board)):
                found2 = True
                x2 = random_x
                y2 = random_y

        p1 = get_piece(x1, y1, board)
        p2 = get_piece(x2, y2, board)

        place_piece(p1, x2, y2, board)
        place_piece(p2, x1, y1, board)
        clear()
        print_board(board)
